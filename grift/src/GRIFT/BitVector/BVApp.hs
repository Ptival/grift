...
evalBVAppM eval (IteApp _ eTest eT eF) = do
  cond <- BV.testBit (knownNat :: NatRepr 0) <$> eval eTest
...
bvInteger = bvExpr . BV.mkBV knownNat
...
-- | Zero-extension
zextE :: (BVExpr expr, KnownNat w', (w + 1) <= w') => expr w -> expr w'
...
-- | Sign-extension, identity on same size
sextE :: forall expr w w'.
  (BVExpr expr, KnownNat w, KnownNat (w + 1), KnownNat w', 1 <= w, w <= w') =>
  expr w -> expr w'
sextE e =
  case testStrictLeq @w @w' knownNat knownNat of
    Left LeqProof -> appExpr (SExtApp @w @w' knownNat knownNat e)
    Right Refl -> e
