...
singleChunk :: (KnownNat w, KnownNat w') => Natural -> BitLayout w w'
...
-- at a given position.
bvOrAt :: (KnownNat t, 1 <= t, s + 1 <= t) => Natural -> BV.BV s -> BV.BV t -> BV.BV t
bvOrAt start s =
  BV.or (BV.ashr knownNat (BV.zext knownNat s) start)

type family KnownNats :: List * -> Constraint where
  KnownNats '[] = ()
...
bvOrAtAll ::
  ...
  BV.BV s ->
  BV.BV t
bvOrAtAll _ tRepr [] _ = BV.mkBV tRepr 0
bvOrAtAll sRepr tRepr (Some (Chunk chunkRepr chunkStart) : chunks) sVec =
  bvOrAt chunkStart (BV.truncBits chunkWidth sVec) (bvOrAtAll sRepr tRepr chunks (BV.shl sRepr sVec (- chunkWidth)))
  where
...
layoutLens :: BitLayout t s -> Lens' (BV.BV t) (BV.BV s)
layoutLens layout = lens (extract layout) (inject layout)

-- | Lens for a parameterized 'List' of 'BitLayout's.
layoutsLens :: forall ws. List (BitLayout 32) ws -> Lens' (BV.BV 32) (List BV.BV ws)
layoutsLens layouts =
  lens
