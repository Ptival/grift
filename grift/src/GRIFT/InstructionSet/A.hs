...
-- | A extension (RV64)
a64 :: (KnownRV rv, 32 <= RVWidth rv, 64 <= RVWidth rv, AExt << rv) => InstructionSet rv
...
aSemantics :: forall rv . (32 <= RVWidth rv, KnownRV rv, AExt << rv) => SemanticsMap rv
...
amoOp32 :: (KnownRV rv, 32 <= RVWidth rv)
...
amoOp64 :: (KnownRV rv, 64 <= RVWidth rv)
